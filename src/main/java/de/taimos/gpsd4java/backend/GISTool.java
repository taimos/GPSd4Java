...
public final class GISTool {
  ...
  public static double getDistance(final TPVObject tpv1, final TPVObject tpv2) {
    return GISTool.getDistance(
        tpv1.getLongitude(), tpv2.getLongitude(), tpv1.getLatitude(), tpv2.getLatitude());
  }
  ...
  public static double getDistance(
    ...
    final double a =
        Math.pow(Math.sin(dlat / 2), 2)
            + (Math.cos(y1rad) * Math.cos(y2rad) * Math.pow(Math.sin(dlong / 2), 2));
