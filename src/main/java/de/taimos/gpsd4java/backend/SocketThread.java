...
public class SocketThread extends Thread {
   ...
   * @param daemon whether to configure the thread as a daemon, as defined in {@link
   *     Thread#setDaemon}
   */
  public SocketThread(
      final BufferedReader reader,
      final GPSdEndpoint endpoint,
  ...
  public void run() {
    ...
    if (this.running.get() && !Thread.interrupted()) {
      if (this.reader != null) {
        SocketThread.LOG.warn(
            "Problem encountered while reading/parsing/handling line, attempting restart");
      }
      retry();
    }
  }
