...
public class GPSdEndpoint {
   ...
   * @param daemon whether to start the underlying socket thread as a daemon, as defined in {@link
   *     Thread#setDaemon}
   */
  public GPSdEndpoint(
      final String server,
      final int port,
      final AbstractResultParser resultParser,
   ...
   * @throws IOException
   */
  public GPSdEndpoint(final String server, final int port, final AbstractResultParser resultParser)
   ...
   */
  public WatchObject watch(final boolean enable, final boolean dumpData)
   ...
   * @throws JSONException
   */
  public WatchObject watch(final boolean enable, final boolean dumpData, final String device)
      throws IOException, JSONException {
  ...
  private <T extends IGPSObject> T syncCommand(final String command, final Class<T> responseClass)
    ...
    synchronized (this.asyncMutex) {
      ...
      while (true) {
        ...
        // FIXME possible infinite loop if expected result arrives but new result overrides expected
        // result before getting to this point.
   ...
   *
   * <p>see: https://lists.gnu.org/archive/html/gpsd-dev/2015-06/msg00001.html
   *
